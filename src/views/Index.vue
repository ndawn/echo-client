...
export default {
  ...
  beforeMount () {
    ...
    //     {pk: 1, subnet: {pk: 1, cidr: '192.168.0.0/24'}}
    //   ]),
    //   this.$store.dispatch('setSubnets', [
    //     {pk: 1, cidr: '192.168.0.0/24'},
    //     {pk: 2, cidr: '192.168.1.0/24'},
    //     {pk: 3, cidr: '192.168.2.0/24'},
    //     {pk: 4, cidr: '192.168.3.0/24'},
    //     {pk: 5, cidr: '192.168.4.0/24'}
    //   ])
    // ]).then(() => {
    ...
    axios.get('/api/agents/', {headers: {Authorization: `Bearer ${localStorage.token}`}}).then(response => {
      this.$store.dispatch('setAgents', response.data);
    }).catch(error => {
      if (error.response.status === 401 || (error.response.status === 422 && error.response.data.detail === 'Signature has expired')) {
        delete localStorage.token;
        this.$router.push('/login');
